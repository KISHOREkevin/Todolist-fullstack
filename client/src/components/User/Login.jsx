...
const Login = ()=>{
    ...
    let submitHandler = async (e)=>{
        try {
            e.preventDefault();
            await axios.post("http://localhost:5000/equals",input).then((response)=>{
                if(response.status === 200 ){
                    setError({indicater:false,message:""});
                    navigate("/tasks/"+response.data.user._id);
                }else{
                    navigate("/log-in");
                }
            });
        ...
        <>
            ...
            <form onSubmit={submitHandler}>
                <div className="form-username">
                    ...
                    <TextField onChange={inputHandler} value={input.username} sx={{width:"100%",input:{color:"#F79BD3"}}}  label="Enter name" variant="outlined" type="text" name="username" id="username" required /> <br /><br />
                </div>
                ...
                <Button type="submit" variant="contained" fullWidth style={{background:"#EA1179",color:"#F79BD3"}} >Log in</Button>
                <Link to={"/sign-up"} style={{color:"yellow",textAlign:"center",display:"block",marginTop:"10px"}}>Doesn't have an account ? sign-up</Link>
            </form>
            </Box>
            </Grow>
        </>
